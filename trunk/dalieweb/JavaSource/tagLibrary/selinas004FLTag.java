	...
	private String writeDokumentDataToPageContext(Vector dokumentLinks) {
	    ...
	    for (int i = 0; i < dokumentLinks.size(); i++){         
			...
			tableTRTD = tableTRTD + "<tr bgcolor='" + farbe[i % 2] + "'>" +
					"<td width='20%' class="+FB+FB+"><a href='"+((HttpServletResponse) pageContext.getResponse()).encodeURL("../LinkToGetServlet?ApplicationsId="+link.getApplicationsId()+ UploadApplication.getApplicationsKz(link.getContentType()))+"' target='_blank' title='"+link.getNotizOfLink()+"'>" + link.getNameOfLinkUn() +"</a></td>" +
